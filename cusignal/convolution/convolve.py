...
def fftconvolve(in1, in2, mode="full", axes=None):
    ...
    in1 = xp.ascontiguousarray(in1)
    in2 = xp.ascontiguousarray(in2)
    noaxes = axes is None
    ...
    axes = xp.asnumpy(axes)

    if not noaxes and not axes.size:
        raise ValueError("when provided, axes cannot be empty")

    if noaxes:
        other_axes = np.array([], dtype=xp.intc)
    else:
    ...
    ) or np.issubdtype(in2.dtype, xp.complexfloating)
    ...
    if not complex_result:
        sp1 = xp.fft.rfftn(in1, fshape, axes=axes)
        sp2 = xp.fft.rfftn(in2, fshape, axes=axes)
        ret = xp.fft.irfftn(sp1 * sp2, fshape, axes=axes)[fslice].copy()
    else:
        sp1 = xp.fft.fftn(in1, fshape, axes=axes)
        sp2 = xp.fft.fftn(in2, fshape, axes=axes)
        ret = xp.fft.ifftn(sp1 * sp2, axes=axes)[fslice].copy()
