...
# anisoplanatic
imag_rebuild, error = pya.varyingDeconvRL_fft(imag, psfmap, iterations, windowSize, windowStep, windowOverlap, gpu=True)

# isoplanatic, central kernel
psfcenter = np.pad(psfmap[2,2,:,:],((986,986),(986,986)))
imag_RL, error_RL = pyd.richardsonLucy(cp.asarray(imag), cp.asarray(psfcenter), iterations=iterations, verbose=False)
...
# anisoplanatic
imag_rebuild = pya.varyingDeconvRL(imag, psfmap, iterations, windowSize, windowStep, windowOverlap, gpu=True)

# isoplanatic, central kernel
psfcenter = psfmap[2,2,:,:]
imag_RL, error_RL = pyd.richardsonLucy_smallKernel(cp.asarray(imag), cp.asarray(psfcenter), iterations=iterations, verbose=False)
imag_RL, error_RL = imag_RL.get(), error_RL.get()
